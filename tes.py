...
def process_large_translation_file(file_path, output_file, batch_size=10000, max_rows=None):
    ...
    print(f"  区切り文字: {'タブ' if separator == '\\t' else separator}")
            ...
            for line_num, line in enumerate(f, 1):
                ...
                if len(batch_data) >= batch_size:
                    ...
                    if len(all_training_data) > 100000:  # 10万件を超えたら一時保存
                        save_batch_to_file(all_training_data, output_file, append=True)
                        all_training_data = []
                        gc.collect()
    ...
    try:
        ...
        try:
            ...
            for i in range(min(num_samples, len(partial_data))):
                print(f"\n--- サンプル {i+1} ---")
                print(f"text: {partial_data[i]['text'][:100]}...")
                print(f"output: {partial_data[i]['output'][:100]}...")
        ...
        for line in f:
            ...
            if line.startswith('{') and line.endswith('}') or line.endswith('},'):
                ...
                try:
                    ...
                    if current_count < train_count:
                        train_data.append(item)
                    elif current_count < train_count + dev_count:
                    ...
                    if current_count % 10000 == 0:
                        print(f"分割進捗: {current_count:,}/{data_count:,}")
                        
                except json.JSONDecodeError:
                    continue
    
    # 各データセットを保存
    for data, filename, name in [
        ...
        print(f"{name}データを {filename} に保存 ({len(data):,}件)")

# 使用例
if __name__ == "__main__":
    # 指定されたファイルを処理
    input_file = "/home/yukaalive/2025workspace/task_vectors/4_icl_task_vectors/data/translation/raw"
    output_file = "translation_training_data.json"
    
    print("=== 大容量翻訳ファイル処理ツール ===")
    
    # オプション: 最初の100万行だけ処理する場合（テスト用）
    print("\n処理オプション:")
    print("1. 全データを処理")
